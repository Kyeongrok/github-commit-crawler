...
class GitWebClientTest {
    ...
    void getLatestRepository() {
        Repository latest = gitWebClient.getLatestRepository("menuhwang");
    ...
    void getLatestRepositoryNotFoundUser() {
        assertThrows(RuntimeException.class, () -> gitWebClient.getLatestRepository("menuhwangddddd"));
    }

    @Test
    @DisplayName("리포지토리 커밋 리스트 불러오기 : 페이징 기본 설정")
    void getRepositoryCommitDetail() {
        Repository repository = gitWebClient.getLatestRepository("menuhwang");
        List<CommitDetail> find = gitWebClient.getRepositoryCommitDetails(repository);
        System.out.println(find.size());
        assertTrue(find.size() <= GitWebClient.DEFAULT_PER_PAGE);  // 최신 리포지토리의 경우 커밋 수가 요청 보다 적을 수 있기 때문
    }
    ...
    void getRepositoryCommitDetailWithPaging() {
        ...
        final int PAGE = 1;
        Repository repository = gitWebClient.getLatestRepository("menuhwang");
        List<CommitDetail> find = gitWebClient.getRepositoryCommitDetails(repository, PER_PAGE, PAGE);
        System.out.println(find.size());
